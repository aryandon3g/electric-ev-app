        ...
        <main className="flex-1 overflow-y-auto px-6 pb-28 pt-2 space-y-4 [&::-webkit-scrollbar]:hidden" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          ...
          {activeTab === 'dashboard' && (
            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col items-center h-full space-y-4">
              ...
              {/* JK BMS Connection Assistant & Protocol Troubleshooter */}
              <div className="w-full bg-white rounded-3xl p-5 shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-gray-100 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h3 className="text-xs font-extrabold text-gray-900 uppercase tracking-wider">JK BMS Connection Fixer</h3>
                      <p className="text-[10px] text-gray-400">Force wake, protocol switcher & live raw bytes</p>
                    </div>
                  </div>
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-50 text-blue-600">
                    {bmsData.detectedProtocol || 'JK BMS Mode'}
                  </span>
                </div>

                {/* Force Protocol Selection Buttons */}
                <div className="flex flex-wrap gap-1.5 pt-1">
                  <button
                    onClick={() => setManualProtocol('JK BMS')}
                    className={`px-2.5 py-1.5 rounded-xl text-[11px] font-extrabold transition-all ${
                      bmsData.detectedProtocol === 'JK BMS'
                  ...
                  <button
                    onClick={() => setManualProtocol('JBD/Xiaoxiang')}
                    className={`px-2.5 py-1.5 rounded-xl text-[11px] font-extrabold transition-all ${
                      bmsData.detectedProtocol === 'JBD/Xiaoxiang'
                        ? 'bg-blue-600 text-white shadow-sm'
                        : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                    }`}
                  >
                  ...
                  <button
                    onClick={() => setManualProtocol('Daly')}
                    className={`px-2.5 py-1.5 rounded-xl text-[11px] font-extrabold transition-all ${
                      bmsData.detectedProtocol === 'Daly'
                        ? 'bg-blue-600 text-white shadow-sm'
                        : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                    }`}
                  >
                  ...
                  <button
                    onClick={() => setManualProtocol('ANT BMS')}
                    className={`px-2.5 py-1.5 rounded-xl text-[11px] font-extrabold transition-all ${
                      bmsData.detectedProtocol === 'ANT BMS'
                        ? 'bg-blue-600 text-white shadow-sm'
                        : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                    }`}
                  >
                ...
                {/* Action Buttons: Force Wake + Poll & Simulate Test Packet */}
                <div className="grid grid-cols-2 gap-2 pt-1">
                  <button
                    onClick={triggerPollNow}
                    className="w-full py-2.5 px-3 bg-blue-50 text-blue-600 hover:bg-blue-100 border border-blue-100 rounded-2xl font-bold text-xs flex items-center justify-center gap-1.5 transition-all shadow-xs active:scale-98"
                  >
                    <RefreshCw size={14} className="text-blue-500" />
                    <span>Force Send JK Wake</span>
                  </button>
                  <button
                    onClick={() => simulateIncomingPacket('JK_READ_ALL_INFO')}
                    className="w-full py-2.5 px-3 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-100 rounded-2xl font-bold text-xs flex items-center justify-center gap-1.5 transition-all shadow-xs active:scale-98"
                  >
                    <Zap size={14} className="text-emerald-600" />
                    <span>Test Demo JK Data</span>
                  </button>
                </div>

                {/* Live Raw BLE Stream Terminal Toggle */}
                <div className="pt-2 border-t border-gray-100 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Terminal size={14} className="text-gray-400" />
                    <span className="text-[11px] font-bold text-gray-600">Raw Hex Log Terminal ({bmsData.totalPacketsReceived || 0} pkts)</span>
                  </div>
                  <button
                    onClick={() => setShowTerminal(!showTerminal)}
                    className="text-[11px] font-extrabold text-blue-600 bg-blue-50 px-2.5 py-1 rounded-lg hover:bg-blue-100 transition-colors"
                  >
                    {showTerminal ? 'Hide Console' : 'Show Console'}
                  </button>
                </div>

                {/* Expandable Live Dark Console Terminal */}
                {showTerminal && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="pt-2">
                    <div className="bg-slate-950 text-emerald-400 rounded-2xl p-3 font-mono text-[10px] space-y-1.5 max-h-56 overflow-y-auto border border-slate-800 shadow-inner">
                      <div className="flex items-center justify-between pb-1.5 border-b border-slate-800 text-slate-400 text-[9px] uppercase tracking-wider font-sans">
                        <span>Live BLE Packet Stream</span>
                        <div className="flex items-center gap-2">
                          {bmsData.lastPacketTime && <span>Last: {bmsData.lastPacketTime}</span>}
                          <button onClick={clearHexLogs} className="text-red-400 hover:text-red-300 font-bold flex items-center gap-0.5">
                            <Trash2 size={10} /> Clear
                          </button>
                        </div>
                      </div>

                      {bmsData.rawHexLogs.length === 0 ? (
                        <div className="text-slate-600 text-center py-4 font-sans text-xs">
                          No packets received yet. Connect BLE or tap "Force Send JK Wake".
                        </div>
                      ) : (
                        bmsData.rawHexLogs.slice(0, 30).map(log => (
                          <div key={log.id} className="leading-tight border-b border-slate-900/60 pb-1">
                            <span className={log.type === 'RX' ? 'text-cyan-400 font-bold' : log.type === 'TX' ? 'text-amber-400 font-bold' : 'text-slate-500'}>
                              [{log.timestamp}] {log.type}:
                            </span>{' '}
                            <span className="text-slate-200">{log.hex}</span>
                            {log.decodedInfo && <div className="text-emerald-400 text-[9px] pl-2 opacity-90">{log.decodedInfo}</div>}
                          </div>
                        ))
                      )}
                    </div>
